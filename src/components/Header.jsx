      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-center items-center py-4">
          ...
          <nav className="flex space-x-1 bg-gray-100 dark:bg-slate-700 rounded-full p-1">
            {['기술', '경력', '프로젝트', '블로그'].map((tab, index) => (
              <button
                key={tab}
                className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${
                  index === 0 
                    ? 'bg-blue-500 dark:bg-blue-400 text-white shadow-lg dark:shadow-blue-400/20'
                    : 'text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-600'
                }`}
              >
                {tab}
              </button>
